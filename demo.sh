...
demo_authentication() {
    ...
    print_step "SCENARIO: Banking User Authentication"
    print_info "Demonstrating secure JWT-based authentication with role-based access control"
    echo
    
    print_talking_point "Authentication Overview:"
    ...
    echo ""
    echo "Each role has different permission levels and access restrictions.\""
    echo
    
    print_talking_point "Test Script Details:"
    echo "Script: ./test_phase1.sh"
    echo "Tests Include:"
    echo "• Valid user login with correct credentials"
    echo "• JWT token validation and parsing"
    echo "• Role-based permission verification"
    echo "• Invalid credential rejection"
    echo "• Token expiration handling"
    echo
    
    print_step "Sample Authentication Request:"
    echo "POST /auth/login"
    echo "Content: {\"username\": \"customer1\", \"password\": \"password123\"}"
    echo "Expected: JWT token with customer role and account access"
    echo
    ...
    echo
    print_talking_point "Key Security Features Demonstrated:"
    echo "• Secure password validation (bcrypt hashing)"
    echo "• JWT tokens with role-based claims"
    echo "• Token expiration (30-minute default)"
    echo "• Account-scoped permissions"
    echo "• Invalid credential protection"
    echo
    
    print_success "Authentication Demo Complete!"
    print_info "Result: All authentication flows working with proper security controls"
    echo
    wait_for_user
}
